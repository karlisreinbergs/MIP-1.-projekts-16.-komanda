...
def spele(isHumanChoise: bool, virknes_garums: int, algorithm: str):
      ...
      def computer_move():
            ...
            if not current_state.ir_cilveka_gajiens:
                  root = TreeNode(current_state)
                  best_node = best_move(root, 3, algorithm)
                  if best_node:
...
def initApp():
      ...
      def open_algorithm_window(virknes_garums: int):
            ...
            Radiobutton(algo_window, text="Minimax", variable=algorithm, value="minimax").pack()
            Radiobutton(algo_window, text="Alpha-Beta", variable=algorithm, value="alphabeta").pack()
        
            def submit_algorithm():
                  algo_window.destroy()
                  open_pirmais_gajiens_window(virknes_garums, algorithm.get())
        
            Button(algo_window, text="Turpināt", command=submit_algorithm).pack(pady=10)
      ...
      def open_pirmais_gajiens_window(virknes_garums: int, algorithm: str):
            ...
            Radiobutton(choice_window, text="Datoram", variable=choice, value="Desktop").pack()
            Radiobutton(choice_window, text="Cilvēkam", variable=choice, value="User").pack()
